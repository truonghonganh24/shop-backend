...
# ===================== CONFIG =====================
# ✅ DEPLOY READY:
# - Render sẽ set DATABASE_URL (Postgres)
# - Local không có DATABASE_URL thì fallback SQLite ./app.db
raw_db_url = os.getenv("DATABASE_URL", "sqlite:///./app.db")

# ✅ Fix cho Render: thường DATABASE_URL bắt đầu bằng postgres://
...
ADMIN_EMAIL = "anhchinhlahonga311206@gmail.com"
ADMIN_PASSWORD = "Honganh123@123A"  # bcrypt giới hạn 72 bytes

BANNED_LOGIN_MESSAGE = "Tài khoản của bạn đã bị cấm"
...
# ===================== APP =====================
app = FastAPI(title="Shop Simplified Backend")
...
def serve_html(filename: str):
    ...
    if not file_path.exists():
        raise HTTPException(status_code=404, detail=f"Thiếu file static/{filename}")
...
def login(data: LoginSchema, request: Request, db: Session = Depends(get_db)):
    ...
    # ADMIN chỉ login từ localhost
    client_ip = request.client.host
    if user.role == ROLE_ADMIN and client_ip != "127.0.0.1":
        raise HTTPException(status_code=403, detail="Admin chỉ được đăng nhập từ máy chủ")
...
def delete_user_post(data: AdminActionSchema, admin: User = Depends(require_admin), db: Session = Depends(get_db)):
    u = db.query(User).filter(User.email == data.email).first()
    if not u:
        raise HTTPException(status_code=404, detail="Không tìm thấy user")
    if u.role == ROLE_ADMIN:
    ...
    db.delete(u)
    db.commit()
    return {"message": f"Đã xóa tài khoản {data.email}"}
...
def delete_user_delete(
    ...
    db.delete(u)
    db.commit()
    return {"message": f"Đã xóa tài khoản {email}"}
